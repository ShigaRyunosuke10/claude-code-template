...
```bash
Task:deployment-agent(prompt: "デプロイ要件定義を支援")
```
...
```bash
Task:deployment-agent(prompt: "Phase 0.1のデプロイ計画に基づいて設定ファイルを生成")
```
...
```bash
Task:deployment-agent(prompt: "Phase 0で選択したプラットフォームの環境変数設定を検証")
```
...
```bash
Task:deployment-agent(prompt: "デプロイエラー解析: {{ERROR_MESSAGE}}")
```
...
### deployment-agent（統合デプロイエージェント）

**旧 infra-validator + deploy-manager を統合**

- **Task 0**: デプロイ要件定義・プラットフォーム推奨
- **Task 1**: 設定ファイル自動生成（Dockerfile, CI/CD, .env等）
- **Task 2**: 環境変数検証・セキュリティチェック（デプロイ前検証）
- **Task 3**: 初回デプロイ実行
- **Task 4**: CI/CD設定（継続的デプロイ）
- **Task 5**: デプロイ後ヘルスチェック
- **Task 6**: ロールバック
- **Task 7**: トラブルシューティング（エラー解析）
